...
class Processor:
    ...
    def process(self, commands):
        for command in commands:
            if "Add" == command[0]:
                self._add(name=command[1], card_number=command[2], limit=command[3])
            elif "Charge" == command[0]:
                self._charge(name=command[1], amount=command[2])
            elif "Credit" == command[0]:
                self._credit(name=command[1], amount=command[2])
    ...
    def _charge(self, name, amount):
        ...
        for i in range(0, len(self.accounts[name])):
            if leftover <= 0:
                break
            capacity = (
                self.accounts[name][i]["limit"] - self.accounts[name][i]["balance"]
            )
            if capacity < leftover:
                self.accounts[name][i]["balance"] = -self.accounts[name][i]["limit"]
